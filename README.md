...
- Discretize into fixed windows (e.g., hourly bins) to produce a **patient-hour trajectory table**, with one row per patient per hour.
...
|---|---|
| `chartevents` | Current value plus short-term trend/slope; rolling mean/std; composite early-warning score (MEWS/NEWS-style); observation frequency |
| `labevents` | Most recent value plus abnormal flag; rate of change between draws; count of currently abnormal labs; time since last draw |
...
| Patient | Task | Severity (1 to 10) | Time Pressure (1 to 10) | Priority Score (Severity × Time) |
|---|---|---|---|---|
...
**Ranked order:** Patient B (64), then Patient D (63), then Patient A (54), then Patient C (42). Severity and time-pressure values can be sourced from a **rule-based checklist** (e.g., "if potassium > 6.0, severity = 9", simple and explainable, but rigid) or **learned from historical outcome data** (more flexible, feeds directly into §3.4).

### 3.4 Modeling Approach

**Primary approach: Gradient-boosted trees plus learning-to-rank**
- Stage 1: a discrete-time or survival model over windowed features predicts probability (or time-to-event) of a clinically meaningful adverse event within a short horizon (e.g., 1 to 4 hours).
...
- **Irregular/informative sampling**: measurement frequency correlates with perceived severity, so encode time-since-observation and frequency as features rather than imputing naively.
- **Non-random missingness**: a missing value often reflects a clinical decision, not randomness, so use masking plus time-decay (GRU-D) instead of mean-fill imputation.
- **Confounding by clinical response**: clinicians already act on perceived risk, so the data reflects treated, not natural, trajectories; careful feature-time alignment and causal caution are needed when interpreting associations.
- **Label scarcity / class imbalance**: no ground-truth urgency label exists and adverse events are rare, so use well-defined objective outcomes, imbalance-aware metrics (AUPRC), and clinician-annotated validation samples.
- **Timestamp leakage**: delayed/batch documentation can leak future information, so enforce strict point-in-time feature construction.
- **Cross-source standardization**: inconsistent units, coding, and naming across labs/medications require a standardization/mapping layer (e.g., to LOINC/RxNorm) built early in the pipeline.
- **Generalizability**: case mix, staffing, and EHR systems differ across units/sites, so validate externally and monitor for performance drift.
- **Privacy/governance**: patient-level longitudinal data requires strict controls, including de-identification, access control, and IRB oversight throughout.
